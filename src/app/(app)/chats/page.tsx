    ...
    <div className="px-4 py-4">
      ...
      {myMatches.length === 0 ? (
        ...
        <div className="space-y-2 mb-5">
          {myMatches.map((m) => (
            <Link key={m.id} href={`/chats/${m.id}`} className="flex items-center gap-3 rounded-xl bg-white p-3 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 text-[11px] text-gray-500">
                  <span className="flex items-center gap-0.5"><Clock size={10} /> {m.start_time}</span>
                  <span className="flex items-center gap-0.5"><Users size={10} /> {m.filled_slots}/{m.max_slots}</span>
                </div>
              </div>
              <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${m.status === 'OPEN' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{m.status}</span>
            </Link>
          ))}
        </div>
      )}

      <h2 className="text-sm font-semibold text-gray-500 uppercase mb-2">Yêu cầu đã gửi</h2>
      {myRequests.length === 0 ? (
